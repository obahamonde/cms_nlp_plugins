...
from backend import (create_thread, use_chat, use_chat_stream, use_function,
                 use_image, use_instruction, use_instruction_stream, use_tts,
                 use_vision)
from backend.routes.files import app as files_app
from backend.routes.threads import app as threads_app
...
async def tts_endpoint(
    ...
    """
    result = await use_chat(ai=ai, text=text, context=context,temperature=temperature, max_tokens=max_tokens, model=model)
    audio = use_tts(ai=ai,text=result, voice=voice, model=voice_model,response_format=response_format)
...
async def autocomplete_blog_endpoint(
    ...
    response = await use_instruction(ai=ai, text=text, max_tokens=max_tokens, temperature=temperature, model=model)
...
async def chat_endpoint(text:str):
    ...
    """
    response = use_chat_stream(ai=ai, text=text,context=None,temperature=0.7, max_tokens=1024, model="gpt-4-1106-preview")
    async def generator():
...
app.include_router(threads_app)
app.include_router(files_app)